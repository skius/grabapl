...
fn bubble_sort_op(op_id: OperationId) -> UserDefinedOperation<TestSemantics> {
    ...
    builder.expect_shape_edge(p0, child, EdgeType::Wildcard).unwrap();

    builder.enter_true_branch().unwrap();
    // if we have a child, check if p0 > child
    builder.start_query(TestQuery::CmpFstSnd(Greater), vec![p0, child]).unwrap();
    builder.enter_true_branch().unwrap();
    // if p0 > child, swap values
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::SwapValues), vec![p0, child]).unwrap();
    builder.end_query().unwrap();
    // continue with the child
    builder.add_operation(BuilderOpLike::Recurse, vec![child]).unwrap();
...
fn bubble_sort_op_2(op_id: OperationId) -> UserDefinedOperation<TestSemantics> {
    ...
    builder.start_shape_query("query").unwrap();
    builder.expect_shape_node("parent".into(), NodeType::Integer).unwrap();
    let parent = AbstractNodeId::dynamic_output("query", "parent");
    builder.expect_shape_edge(parent, p0, EdgeType::Wildcard).unwrap();

    builder.enter_true_branch().unwrap();
    // if we have a parent, check if parent > p0
    builder.start_query(TestQuery::CmpFstSnd(Greater), vec![parent, p0]).unwrap();
    builder.enter_true_branch().unwrap();
    // if parent > p0, swap values
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::SwapValues), vec![parent, p0]).unwrap();
    builder.end_query().unwrap();
    // continue with the parent
    builder.add_operation(BuilderOpLike::Recurse, vec![parent]).unwrap();
...
fn list_len_op(op_id: OperationId) -> UserDefinedOperation<TestSemantics> {
    ...
    let p0 = AbstractNodeId::param("p0");
    // output node
    builder.expect_parameter_node("output", NodeType::Integer).unwrap();
    let output = AbstractNodeId::param("output");
    // p0 exists, so increment output
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::AddInteger(1)), vec![output]).unwrap();
    ...
    builder.expect_shape_edge(p0, child, EdgeType::Wildcard).unwrap();
    builder.enter_true_branch().unwrap();
    builder.add_operation(BuilderOpLike::Recurse, vec![child, output]).unwrap();
    builder.end_query().unwrap();
...
fn bubble_sort_n_times_op(op_id: OperationId) -> UserDefinedOperation<TestSemantics> {
    ...
    builder.expect_parameter_node("n", NodeType::Integer).unwrap();
    let n = AbstractNodeId::param("n");

    // if n == 0, return
    builder.start_query(TestQuery::ValueEqualTo(NodeValue::Integer(0)), vec![n]).unwrap();
    builder.enter_true_branch().unwrap();
    ...
    builder.end_query().unwrap();
    // decrement n
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::AddInteger(-1)), vec![n]).unwrap();
    // continue with the child
    builder.add_operation(BuilderOpLike::Recurse, vec![child, n]).unwrap();

    builder.build(op_id).unwrap()
}

fn main_bubble_sort_op(op_id: OperationId, n_times_op_id: OperationId, op_ctx: &OperationContext<TestSemantics>) -> UserDefinedOperation<TestSemantics> {
    let mut builder = OperationBuilder::new(op_ctx);
    // first node
    builder.expect_parameter_node("p0", NodeType::Integer).unwrap();
    let p0 = AbstractNodeId::param("p0");
    ...
    builder.add_named_operation("new".into(), BuilderOpLike::Builtin(TestOperation::AddNode {
    ...
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::CopyValueFromTo), vec![n, new_node]).unwrap();
    // run bubble sort n times
    builder.add_operation(BuilderOpLike::FromOperationId(n_times_op_id), vec![p0, new_node]).unwrap();
    // remove new_node
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::DeleteNode), vec![new_node]).unwrap();
    // decrement n
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::AddInteger(-1)), vec![n]).unwrap();
    // recurse on self
    builder.add_operation(BuilderOpLike::Recurse, vec![p0, n]).unwrap();

    builder.build(op_id).unwrap()
}

fn wrap_main_bubble_sort_op(op_id: OperationId, main_bubble_sort_op_id: OperationId, list_len_op_id: OperationId, op_ctx: &OperationContext<TestSemantics>) -> UserDefinedOperation<TestSemantics> {
    let mut builder = OperationBuilder::new(op_ctx);
    // first node
    builder.expect_parameter_node("p0", NodeType::Integer).unwrap();
    let p0 = AbstractNodeId::param("p0");
    ...
    // get list len
    builder.add_operation(BuilderOpLike::FromOperationId(list_len_op_id), vec![p0, output_node]).unwrap();
    ...
    builder.start_query(TestQuery::ValueEqualTo(NodeValue::Integer(0)), vec![output_node]).unwrap();
    builder.enter_false_branch().unwrap();


    // we want to run it len-1 times
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::AddInteger(-1)), vec![output_node]).unwrap();
    // run main bubble sort operation
    builder.add_operation(BuilderOpLike::FromOperationId(main_bubble_sort_op_id), vec![p0, output_node]).unwrap();
    // remove output node
    builder.add_operation(BuilderOpLike::Builtin(TestOperation::DeleteNode), vec![output_node]).unwrap();
...
fn bubble_sort() {
    ...
    let main_bubble_sort_op = main_bubble_sort_op(main_bubble_sort_op_id, bubble_sort_n_times_op_id, &op_ctx);
    op_ctx.add_custom_operation(main_bubble_sort_op_id, main_bubble_sort_op);

    let wrap_main_bubble_sort_op_id = 5;
    let wrap_main_bubble_sort_op = wrap_main_bubble_sort_op(wrap_main_bubble_sort_op_id, main_bubble_sort_op_id, list_len_op_id, &op_ctx);
...
fn sort_using_grabapl(values: &[i32], op_ctx: &OperationContext<TestSemantics>, bubble_sort_op_id: OperationId) -> Vec<i32> {
    ...
    for i in 0..node_keys_ordered.len() - 1 {
        g.add_edge(node_keys_ordered[i], node_keys_ordered[i + 1], "".to_string());
    }
    // run the operation
