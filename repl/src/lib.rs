...
pub fn parser<'a>() -> impl Parser<'a, ParserInput<'a>, Program, ParserError<'a>> {
    ...
    fn token<'a, P: Clone + Parser<'a, ParserInput<'a>, T, ParserError<'a>>, T>(
        p: P,
    ...
    let call = ident
        ...
        .then(args)
        .map(
            |((func, op), args): ((&str, Option<&str>), Vec<Expr>)| match op {
    ...
    let shape_pattern = {
        let node = token(text::keyword("node"))
            .ignore_then(ident.clone())
            .then_ignore(token(just(':')))
            .then(ident.clone())
        ...
        let edge = token(text::keyword("edge"))
            .ignore_then(ident.clone())
            .then_ignore(token(just("->")))
            .then(ident.clone())
            .then_ignore(token(just(':')))
            .then(ident.clone())
            .map(
                |((src, dst), ty): ((&str, &str), &str)| ShapePattern::Edge {
                    src: src.to_string(),
                    dst: dst.to_string(),
                    ty: ty.to_string(),
                },
    ...
    let if_stmt = recursive(|if_stmt| {
        token(text::keyword("if"))
            .ignore_then(condition)
            .then(block.clone())
            .then(
                token(text::keyword("else"))
                    .ignore_then(
                        if_stmt
                            .map(|s| Block(vec![Statement::If(s)]))
                            .or(block.clone()),
                    )
                    .or_not(),
            )
            .map(|((condition, true_branch), false_branch)| IfStmt {
                condition,
                true_branch,
                false_branch,
            })
    ...
    let return_stmt = token(text::keyword("return"))
        .ignore_then(
            ident
                .clone()
                .then_ignore(token(just(':')))
    ...
    let func_def = token(text::keyword("def"))
        .ignore_then(ident.clone())
        ...
        .map(
            |(((name, params), ret), body): (
                ((&str, Vec<TypedParam>), Option<Vec<TypedParam>>),
                Block,
            )| FuncDef {
                name: name.to_string(),
                params,
